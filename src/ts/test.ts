...
const produitAlimentaire1 = new Alimentaire("Pommes", 5);
const produitChimique1 = new Chimique("Acide Sulfurique", 10, "Élevé");
const produitFragile1 = new Fragile("Vase en cristal", 2);
const produitIncassable1 = new Incassable("Barre de fer", 50);
const produitAlimentaire2 = new Alimentaire("Bananes", 3);
const produitChimique2 = new Chimique("Désinfectant", 8, "Moyen");
const produitFragile2 = new Fragile("Miroir", 7);
const produitIncassable2 = new Incassable("Poutre en acier", 100);
...
const cargaisonAerienne = new Aerienne("AER-001", 1000, "Paris", "New York", 6000);
const cargaisonMaritime = new Maritime("MAR-001", 50000, "Dakar", "Marseille", 4500);
const cargaisonRoutiere = new Routiere("ROU-001", 5000, "Lyon", "Berlin", 1000);
...
const cargaisonTestPleine = new Aerienne("AER-FULL", 500, "A", "B", 100);
for (let i = 0; i < 10; i++) {
  cargaisonTestPleine.ajouterProduit(new Alimentaire(`Produit ${i+1}`, 1));
}
cargaisonTestPleine.ajouterProduit(new Alimentaire("Produit 11", 1)); // Should be rejected (full)

const cargaisonTestFermee = new Aerienne("AER-CLOSED", 500, "C", "D", 100, "en-cours", "ferme");
cargaisonTestFermee.ajouterProduit(new Alimentaire("Produit Fermé", 1)); // Should be rejected (closed)

const cargaisonTestRouvrir = new Aerienne("AER-REOPEN", 500, "E", "F", 100, "arrive", "ferme");
